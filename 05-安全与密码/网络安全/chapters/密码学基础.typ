...
#note[
  密码学的核心本质是：#emphasis[在不安全的环境中建立信任]。
]

== 核心安全目标

在信息安全领域，密码学通常被要求实现以下四个核心目标（即 #emphasis[CIA + A/N] 模型）：
...
#highlight[“即使密码系统的所有细节（除了密钥）都已公开，系统也应该是安全的。”]
...
#plain-table(
	("阶段", "时间", "技术特征", "典型代表"),
	("古典密码", "古代~19世纪末", "手工代换与置换", "凯撒、维吉尼亚"),
	("近代密码", "20世纪初~1950s", "机械/机电密码机", "Enigma、Vernam"),
	("现代密码", "1949年至今", "数学化、可证明安全、标准化", "DES/AES/RSA/ECC"),
)
...
num = 0x12345678
be = struct.pack(">I", num)  # b'\x12\x34\x56\x78'
le = struct.pack("<I", num)  # b'\x78\x56\x34\x12'
print(be, le)
```
...
data = b"secret"
hex_data = data.hex()                     # 736563726574
b64_data = base64.b64encode(data).decode()  # c2VjcmV0
print(hex_data, b64_data)
```
