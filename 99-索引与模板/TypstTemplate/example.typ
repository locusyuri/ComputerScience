...
//! 对 AI 的提示:
//! 请特别注意以下几点，以确保生成的文档符合预期的格式和内容要求：
//! 1. 结构层级：文档分为多个部分（part），每部分包含多个章节（chapter）。请确保正确使用 `#part` 和标题语法（`=`、`==` 等）来构建清晰的层级结构。以及四级标题有特殊的用法，详见以下对应位置的说明。
//! 2. Typst 语法: Typst 语法——尤其是数学语法——不同于 Latex, 尤其要注意的一点是, 当 Typst 使用自定义变量时(除了单个字母的变量, 如 $a, x, y$), 为了避免被识别为关键字, 应该加引号, 也就是 $length$应当写成$"length"$, 而不是$length$。
//! 3. Typst 加粗语法: Typst 的加粗语法是 `*加粗内容*`, 而不是 `**加粗内容**`。请确保在需要加粗的文本周围使用单个星号。以及, 请特别注意, 对于关键词通常使用加粗, 而对于关键句子（不一定是整句话）, 则使用强调（`#emphasis`）来区分不同的语义层次。
//! 4. 时间空间复杂度请使用 $O()$ 数学语法。
...
==== 四级标题（Sub-subsection）（特殊用法）

当你要“列举多个并列点”，且每个点都需要 2-5 段解释、表格或代码时，优先使用四级标题，而不是直接用无序列表。

推荐结构：

- 一级标题（`=`）：主题章节
- 二级标题（`==`）：该章节下的主模块
- 四级标题（`====`）：主模块内的并列知识点（每点内容较完整）
...
#note[
  判断标准：如果你发现某个列表项已经写成了“小节长度”，就把它提升为四级标题，文档结构会更清晰。
]
