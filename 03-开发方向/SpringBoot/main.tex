...
% 10.4 响应式安全与测试
% Chapter 11：日志、监控与可观测性
% 11.1 SLF4J 与 Logback 配置（日志级别、滚动策略、异步日志）
% 11.2 结构化日志与 MDC（链路追踪上下文）
% 11.3 Actuator 端点使用（健康、信息、指标、env）
% 11.4 自定义健康检查与指标收集
% 11.5 集成 Micrometer 与 Prometheus（Grafana 可视化）
% 11.6 链路追踪入门（Micrometer Tracing + OpenTelemetry）
% 11.7 分布式追踪实践（Zipkin、Jaeger）
% 11.8 日志聚合与分析（ELK Stack、Loki）
