...
% 3.5 配置类与 @Bean 的配合使用
% Chapter 4：日志与单元测试
% 4.1 日志门面与实现（SLF4J + Logback）
% 4.2 日志配置详解（控制台、文件、日志级别）
% 4.3 单元测试基础（@SpringBootTest、@Test）
% 4.4 集成测试与 Mock 技术（@MockBean、@WebMvcTest、@DataJpaTest）
...
% Chapter 9：常用依赖与工具集成
% 9.1 Lombok 与 MapStruct（简化代码与对象映射）
% 9.2 Guava 与 Apache Commons（工具类增强）
% 9.3 常用 JSON 库（Jackson、Gson）
% 9.4 HTTP 客户端（RestTemplate、WebClient）
% 9.5 API 文档生成（SpringDoc OpenAPI 配置与使用）
% 9.6 接口限流基础（Guava RateLimiter）
% Chapter 10：响应式编程与 WebFlux
% 10.1 响应式编程基础（Reactor 核心：Mono/Flux）
% 10.2 Spring WebFlux 核心组件与路由函数
% 10.3 WebFlux 中的数据访问（R2DBC、MongoDB Reactive）
% 10.4 响应式安全与测试
% Chapter 11：监控与可观测性基础
% 11.1 Actuator 端点使用（健康、信息、指标）
% 11.2 自定义健康检查与指标收集
% 11.3 集成 Micrometer 与 Prometheus
% 11.4 链路追踪入门（Micrometer Tracing + OpenTelemetry）
