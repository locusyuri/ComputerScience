...
// Chapter 2：数据类型与变量 🔶
// 2.1 基本数据类型：整型、浮点型、字符型、布尔型（C vs C++）
// 2.2 修饰符：signed、unsigned、short、long、long long
// 2.3 常量：字面常量、符号常量（#define vs const）
...
// Chapter 4：控制流 🔶
// 4.1 条件语句：if、if-else、嵌套 if、switch-case（C++17 constexpr）
// 4.2 循环语句：while、for、do-while、范围 for（C++11）
// 4.3 跳转语句：break、continue、goto、return
// 4.4 类型转换：隐式转换、显式转换（C 强制转换 vs C++ static_cast/reinterpret_cast）

// Chapter 5：数组与指针 🔶
// 5.1 数组：定义、初始化、多维数组、数组名本质（C vs C++ std::array）
// 5.2 指针基础：声明、取地址（&）、解引用（*）、指针运算
// 5.3 指针与数组：数组名作为指针、指针算术、下标访问
// 5.4 指针与字符串：字符数组、字符串字面值 vs std::string
// 5.5 指针数组与数组指针：int* arr[] vs int (*p)[n]
// 5.6 引用：引用 vs 指针、引用的应用场景（C++ 特有）

// Chapter 6：结构体、联合体与枚举 🔶
// 6.1 结构体：定义、初始化、成员访问、内存对齐（C vs C++）
// 6.2 结构体与函数：值传递、指针传递
// 6.3 结构体与 typedef：简化类型定义
// 6.4 联合体：定义、用途、内存布局
// 6.5 枚举类型：enum、枚举值、枚举与 switch（C++11 强类型枚举）

// Chapter 7：函数与内存管理 🔶
// 7.1 函数基础：声明、定义、调用、参数传递（C vs C++）
// 7.2 函数参数：值传递、地址传递、数组作为参数
// 7.3 函数返回：返回值、返回指针、返回引用（C++）
// 7.4 函数递归：递归调用、递归终止条件
// 7.5 函数指针：声明、使用、回调函数
// 7.6 内联函数：inline、编译展开、注意事项（C++）
// 7.7 内存布局：栈、堆、数据段、代码段
// 7.8 动态内存：malloc/free（C）vs new/delete（C++）
// 7.9 内存错误：野指针、内存泄漏、双重释放

#part("C++ 面向对象与泛型")
// ─────────────────────────────────────────────────────────────────────
// Part 2：C++ 面向对象与泛型
// ─────────────────────────────────────────────────────────────
#include "chapters/面向对象与泛型.typ"

// Chapter 1：面向对象编程 🔶
// 1.1 类与对象：封装、访问控制（public/private/protected）
// 1.2 构造函数：默认构造、参数构造、初始化列表、拷贝构造
// 1.3 析构函数：资源释放、RAII 原则
// 1.4 拷贝赋值运算符与移动语义（C++11）
// 1.5 继承：公有/私有/保护继承、菱形继承、虚继承
// 1.6 多态：虚函数、纯虚函数、抽象类、虚表

// Chapter 2：模板与泛型编程 🔶
// 2.1 函数模板：模板函数、类型参数、模板特化
// 2.2 类模板：模板类、默认模板参数
// 2.3 模板参数：非类型参数、模板模板参数
// 2.4 模板元编程：编译期计算、递归模板
// 2.5 类型萃取：type_traits（C++11）
// 2.6 概念与约束：Concepts（C++20）

// Chapter 3：Lambda 与函数对象 🔶
// 3.1 Lambda 表达式：捕捉列表、参数列表、返回类型、函数体
// 3.2 捕捉方式：值捕捉、引用捕捉、混合捕捉、初始化捕捉（C++14）
// 3.3 mutable 修饰符与 Lambda 的常量性
// 3.4 函数对象（Functor）：仿函数、STL 中的应用
// 3.5 std::function 与 std::bind

#part("C++ 标准库")
// ─────────────────────────────────────────────────────────────────────
// Part 3：C++ 标准库
// ─────────────────────────────────────────────────────────────
#include "chapters/CPP标准库.typ"
...
// Part 5：性能优化与工程实践
// ─────────────────────────────────────────────────────────────
#include "chapters/性能优化与工程实践.typ"
